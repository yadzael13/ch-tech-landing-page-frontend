import { render, screen } from "@testing-library/react";
import { http, HttpResponse } from "msw";
import { describe, expect, it, vi } from "vitest";
import { API_URL, server } from "@/test/msw-server";
import { AuthProvider } from "@/lib/auth/AuthContext";
import EditTestimonialPage from "./page";

vi.mock("next/navigation", () => ({
  useRouter: () => ({ push: vi.fn() }),
  useParams: () => ({ id: "t1" }),
}));

const existingTestimonial = {
  id: "t1",
  author_name: "Jane Doe",
  author_role: "CTO",
  client_id: null,
  project_id: null,
  content: "Great work.",
  rating: 5,
  featured: true,
};

describe("EditTestimonialPage", () => {
  it("loads and prefills the testimonial being edited", async () => {
    server.use(
      http.post("/api/auth/refresh", () =>
        HttpResponse.json({ access_token: "tok" }),
      ),
      http.get(`${API_URL}/testimonials`, () =>
        HttpResponse.json({
          success: true,
          data: [existingTestimonial],
          message: null,
        }),
      ),
      http.get(`${API_URL}/clients`, () =>
        HttpResponse.json({ success: true, data: [], message: null }),
      ),
      http.get(`${API_URL}/admin/projects`, () =>
        HttpResponse.json({ success: true, data: [], message: null }),
      ),
    );

    render(
      <AuthProvider>
        <EditTestimonialPage />
      </AuthProvider>,
    );

    expect(await screen.findByDisplayValue("Jane Doe")).toBeInTheDocument();
  });

  it("shows an error state when the testimonial is not found in the list", async () => {
    server.use(
      http.post("/api/auth/refresh", () =>
        HttpResponse.json({ access_token: "tok" }),
      ),
      http.get(`${API_URL}/testimonials`, () =>
        HttpResponse.json({ success: true, data: [], message: null }),
      ),
    );

    render(
      <AuthProvider>
        <EditTestimonialPage />
      </AuthProvider>,
    );

    expect(
      await screen.findByText("No fue posible cargar el testimonio."),
    ).toBeInTheDocument();
  });
});
